...
public class RedFarPush {
    ...
    public RedFarPush(int type) {
        String jsonString = "";

        try {
            File file = new File(new File("").getAbsolutePath().concat(type == 0 ? "/PathVisualizer/src/main/java/com/example/meepmeep/positions/red/far-basket.json" : "/sdcard/FIRST/positions/blue/far-basket.json"));
    ...
    public Action start(TrajectoryActionBuilder builder) {
        return builder.splineToLinearHeading(this.SCOREPUSH, Math.toRadians((90)))
                .turnTo(Math.toRadians(270))
                .setTangent(Math.toRadians(0))
                .splineToLinearHeading(this.PUSH1, Math.toRadians(0))
                .strafeTo(new Vector2d(46, -55))
                .setTangent(Math.toRadians(90))
                .splineToLinearHeading(this.PUSH2, Math.toRadians(0))
                .strafeTo(new Vector2d(56, -55))
                .setTangent(Math.toRadians(90))
                .splineToLinearHeading(this.PUSH3, Math.toRadians(0))
